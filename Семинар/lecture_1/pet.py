...
#В случае, если значение переменной option равняется 'e', мы запускаем функцию example
        if option == 'e':
            example()

#В случае, если значение переменной option равняется 'p', мы запускаем следующую последовательность операций:
#Если заданный лист пуст, то мы выводим об этом сообщение и возвращаемся в начало цикла.
